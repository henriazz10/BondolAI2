...
class ModelInformation:
	def __init__(self):
		try:
			...
			valid_parameters = []
			for key, value in self.models_info["db_example"].items():
	...
	def extract_models_info(self, parameters: dict = None):
		valid_parameters = ['family', 'technical_name', 'front_name', 'company', 'environment', 'freemium', 'pricing']
		if not parameters:
		...
		for key in valid_parameters:
			if key in parameters:
				extracted_info[key] = parameters[key]
			else:
...
if __name__ == "__main__":
	if requested_action := (input("What do you wanna do? (1: Add a new model, 2: Get model information, 3: Exit) \n")) == "1":
		...
		model_info = ModelInformation()
		model_name = input("Please enter the model's technical name: ").lower()
		info = model_info.get_model_info(model_name)
		if info:
			print(f"Model information for '{model_name}': {info}")
		else:
